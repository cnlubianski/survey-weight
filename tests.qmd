...
generate_data_study3 = function(N, sigma, alpha) {
  X <- runif(N, 0, 0.5)
  epsilon <- runif(N, 0, 0.5)
...
#   # Unweighted Regression --------------
#   X = cbind(1, x_est)
#   betas_u = solve(t(x_est) %*% x_est) %*% t(x_est) %*% y_est
#   y_val_u = x_val %*% betas_u
#   v_u = y_val - y_val_u
#     
#   # Weighted Regression ----------------
#   W =  diag(x = wts_est)
#   betas_w = solve(t(x_est) %*% W %*% x_est) %*% t(x_est) %*% W %*% y_est
#   y_val_w = x_val %*% betas_w
#   v_w = y_val - y_val_w
#   
#   
#   D_bar = mean(v_u^2 - v_w^2)
#   S_D = sd(v_u^2 - v_w^2)
#   Z = (D_bar - 0) / (S_D / sqrt(length(y)))
#   
#   # Standard Z-test
#   D = v_u^2 - v_w^2
#   #Z = (mean(D) - 0) / sd(D)
#   p_value = 2 * (1 - pnorm(abs(Z), mean = 0, sd = 1, lower.tail = TRUE))
#   p_value
# 
#   hist(rnorm(1000))
#   abline(v = Z)
...
for (case in 1:nrow(cases)) {
  DD = PN = HP = PS1 = PS1q = PS2 = PS2q = PS3 = PS3q = WF = LR = rep(NA, B)
  case_storage = data.frame(iteration = seq_len(B), DD, PN, HP, PS1, PS1q,
                            PS2, PS2q, PS3, PS3q, WF, LR)
  for (b in 1:B) {
    ...
    case_storage$PS3q[b] = PS3q_test(data = samp, y = samp$y, x = samp$x, wts = samp$w)
...
reject = results %>%
  mutate(HP = case_when(HP <= 0.05 ~ 1, TRUE ~ 0),
         ...
         PS3q = case_when(PS3q <= 0.05 ~ 1, TRUE ~ 0),
  ...
  mutate(HP = format(round(HP * 100, 1), nsmall = 1),
         ...
         PS3q = format(round(PS3q * 100, 1), nsmall = 1))
...
write.csv(results, "st1_suspicious.csv")
```

```{r}
results = read.csv("st1_suspicious.csv")
...
write.csv(reject_table, "suspicious_sim1.csv")
```
...
columns = c("case", "iteration", "PS3", "PS3wo") ###
mini_results = data.frame(matrix(nrow = 0, ncol = length(columns)))
colnames(mini_results) = columns

for (case in 1:nrow(cases)) {
  PS3 = PS3wo = rep(NA, B) ###
  case_storage = data.frame(iteration = seq_len(B), PS3, PS3wo) ###
  for (b in 1:B) {
    pop = generate_data_study1(N = cases$N[case],
                               sigma = cases$sigma[case],
                               alpha = cases$alpha[case],
    ...
    case_storage$PS3[b] = PS3_test(data = samp, y = samp$y, x = samp$x, wts = samp$w)
    case_storage$PS3wo[b] = PS3_test_wo(data = samp, y = samp$y, x = samp$x, wts = samp$w)
...
reject_mini = mini_results %>% ###
  mutate(PS3 = case_when(PS3 <= 0.05 ~ 1, TRUE ~ 0),
         PS3wo = case_when(PS3wo <= 0.05 ~ 1, TRUE ~ 0)) %>%
  select(-iteration) %>%
  group_by(case) %>%
  summarize(across(everything(), mean))
