...
perm_greg <- function(y, x_s, x_u, wts, B = 1000, replacement = FALSE) {
  ...
  for (b in 1:B) {
    ...
    stat_stor[b] = sum(cbind(1, x_u) %*% betas)
  }
  
  W = diag(wts)
  betas = solve(t(X) %*% W %*% X) %*% t(X) %*% W %*% y
  est_stat = sum(cbind(1, x_u) %*% betas)
  p_o = mean(ifelse(stat_stor >= rep(est_stat, B), 1, 0))
  p_value = 2 * min(p_o, 1 - p_o)
  return(p_value)
...
perm_greg_new <- function(y, x_s, x_u, wts, B = 1000, replacement = FALSE) {
  stat_stor = rep(NA, B)
  X = cbind(1, x_s)
...
hi = perm_greg(y = samp$y, x_s = samp$x, x_u = pop$x, wts = samp$w, B = 10000)
bye = perm_greg_new(y = samp$y, x_s = samp$x, x_u = pop$x, wts = samp$w, B = 10000)
```
...
install.packages("tidycensus") # https://walker-data.com/census-r/an-introduction-to-tidycensus.html
library(tidycensus)

get_decennial()
key = "f08ae5b2480d381a612334b26aaff0b3e1927e61"
