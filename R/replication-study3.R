# ------------------------------------------------------------------------------
# Title: Replication of Wang et al. Study 3
# Description: This script runs the third simulation for the replication 
# chapter which tests the performance of the diagnostic tests with regards 
# to different levels of informative weighting.
# ------------------------------------------------------------------------------
...
cases <- tidyr::expand_grid(N, n, alpha, psi)
...
run_one_rep <- function(N, n, psi, alpha) {
  # Generate population and sample
  pop <- generate_data_study3(N = N, alpha = alpha, psi = psi)
  samp <- generate_sample_poisson(pop, w = pop$w, n = n)
...
reject_table <- sim_results %>%
  ...
  select(n, psi, alpha, DD, PS1, PS1q, PS2, PS2q, WF, HP, PS3)

# Save the results
saveRDS(reject_table, file.path(sim_storage_dir, "replication_reject_table3.rds"))
write.csv(reject_table, file.path(sim_storage_dir, "replication_reject_table3.csv"), row.names = FALSE)
