# ------------------------------------------------------------------------------
# Title: Replication of Wang et al. Study 1
# Description: This script runs the first simulation for the replication 
...
run_one_rep <- function(N, n, sigma, alpha, delta) {
  ...
  pop <- generate_data_study1(N = N, sigma = sigma, alpha = alpha, delta = delta)
...
# Save the results
saveRDS(reject_table, file.path(sim_storage_dir, "replication_reject_table1.rds"))
write.csv(reject_table, file.path(sim_storage_dir, "replication_reject_table1.csv"), row.names = FALSE)
